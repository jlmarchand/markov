---
title: Chaînes de Markov en temps continu
author: "JL Marchand"
...
# Temps continu, espace d'états fini {background-image="https://images.squarespace-cdn.com/content/v1/57d1f16d197aea6d03f08252/1473461284521-JRD0QNGA8YP01V2C3IAQ/Otter_schwimmend_Tusche.gif?format=2500w" background-size="cover" background-opacity="0.4"}
...
::::{.columns}
:::{.column width="50%"}
- l'absence de mémoire d'un temps aléatoire $T$ :
$$\mathbb P (T >t+s|T>t)=\mathbb P(T>s), \quad \forall s,t>0$$
en d'autres termes si on a déjà attendu $t$ unités de temps, la probabilité de devoir attendre encore au moins $s$ unités de temps est exactement la même que dans la situation où on n'a pas encore attendu, il n'y a pas de vieillissement
- par exemple en électronique pour modéliser le temps de vie d'un composant; en considérant deux composants de même nature, l'un ayant déjà servi, les temps restants de vie des deux composants est le même
:::
:::{.column width="50%"}
...
- une seule loi vérifie cette propriété sur $\mathbb R^*$ : la loi **exponentielle** $\mathcal E(\lambda)$, sans surprise car il existe un lien entre les deux lois,
  ...
  + c'est donc de assez naturel que cette loi apparaisse comme le bon candidat !
:::
::::
...
- l'espérance vaut $\frac1\lambda$ et la variance $\frac1{\lambda^2}$
...
- ne pas confondre avec les matrices de transition, ici la somme sur les lignes vaut 0 !

## Chaîne de Markov à temps continu {.unnumbered}
>### Définition 
 le processus $(X_t)_{t\geq0}$ est **une chaîne de Markov à temps continu** de générateur $Q$ de de loi initiale $\mu$ si
> 
>  + la **chaîne incluse** $(Y_n= X_{T_n})_{n\geq 0}$ est une chaîne de Markov *à temps discret* de loi initiale $\mu$ et  de matrice de transition $\Pi$
...
- remarque : $S_n = T_n-T_{n-1}$ (en posant $T_0=0$) en conséquence $T_n = \sum_{k=1}^nS_k$


## Simulation de trajectoires

- on applique la définition, simuler la chaîne incluse revient à simuler une chaîne à temps discret, il ne reste qu'à générer les temps de saut
```{r, warning=FALSE, fig.align='center', out.width = '800px', out.height= '800px', echo=TRUE, eval=FALSE}
traj <- function(mu, Q, H) {
  P <- diag(-1 / diag(Q)) %*% (Q - diag(diag(Q))) #creation de la matrice de transition de la chaîne incluse
  position_courante <- sample(prob = mu, size = 1, x = 1:3) #initialisation de la chaîne
  Y <- c(position_courante) #enregistrement des positions successives
  dernier_temps_saut <- 0 #initialisation des temps de saut
  Tps_Sauts <- c(dernier_temps_saut) #enregistrement des positions successives
  while (dernier_temps_saut < H) {
    S <- rexp(n = 1, rate = -Q[position_courante, position_courante])
    dernier_temps_saut <- dernier_temps_saut + S
    Tps_Sauts <- c(Tps_Sauts , dernier_temps_saut)
    position_courante <- sample(1:length(mu), prob = P[position_courante, ], size = 1)
    Y <- c(Y, position_courante)
  }
  Y[length(Y)] <- Y[length(Y)-1]
  Tps_Sauts[length(Tps_Sauts)] <- H
  df <- data.frame(Tps_Sauts, Y)
  return(df)
}
Q <- matrix(c(-4, 3, 1, 2, -4, 2, 1, 4, -5), ncol = 3, byrow = T)
library(tidyverse)
df <- traj(mu = rep(1 / 3, 3), Q = Q, H = 10)
df %>% 
  ggplot() + 
  aes(x = Tps_Sauts, y = Y) + 
  geom_step(color = "dodgerblue", show.legend = T) + 
  geom_point(size = 2, color =  "dodgerblue3") + 
  xlab("temps") + 
  ylab("état")
```

## Simulation de trajectoires {.unnumbered}

```{r, warning=FALSE, fig.align='center',  out.height= '600px', echo=FALSE, eval=TRUE}
traj <- function(mu, Q, H) {
  P <- diag(-1 / diag(Q)) %*% (Q - diag(diag(Q))) #creation de la matrice de transition de la chaîne incluse
  position_courante <- sample(prob = mu, size = 1, x = 1:3) #initialisation de la chaîne
  Y <- c(position_courante) #enregistrement des positions successives
...
- si on appelle $p_{ij}(t) = \mathbb P(X_t=j|X_0=i)$
  +  pour tout $t\geq 0$, $P(t) = \left(p_{ij}(t)\right)_{i,j\in E}$ est une matrice stochastique et  $$P(t+s)=P(t)P(s)\quad \textrm{et}\quad P(0)=Id.$$
  +  en fait $P'(t)= P(t)Q(t)$ ainsi : $$P(t) = e^{tQ} = \sum_{n=0}^{+\infty}\frac{t^n}{n!}Q^n.$$

:::
:::{.column width="50%"}
  + en temps petit
  $$\mathbb P(X_{t+h}=j|X_t=i) \stackrel{h\to 0}{\sim} q_{ij}h,\quad \textrm{si }i\neq j, \\ \mathbb P(X_{t+h}=i|X_t=i) \stackrel{h\to 0}{\sim} 1+q_{ii}h=1-q_ih$$
:::
::::
...
- on retrouve un équivalent du cas temmps discret, pour que l'intégrale soit finie, il faut nécessairement que $\lim_{t\to+\infty}p_{ii}(t)=0$
...
# Temps continu, espace d'états infini dénombrable {background-image="https://images.squarespace-cdn.com/content/v1/57d1f16d197aea6d03f08252/1473461284521-JRD0QNGA8YP01V2C3IAQ/Otter_schwimmend_Tusche.gif?format=2500w" background-size="cover" background-opacity="0.4"}
...
- l'espace d'état ici est $E = \mathbb N$ est infini dénombrable
...
# Temps discret, espace d'états infini indénombrable {background-image="https://images.squarespace-cdn.com/content/v1/57d1f16d197aea6d03f08252/1473461284521-JRD0QNGA8YP01V2C3IAQ/Otter_schwimmend_Tusche.gif?format=2500w" background-size="cover" background-opacity="0.4"}
...
::::{.columns}
:::{.column width="50%"}
```{r, warning=FALSE, fig.align='center', out.width = '600px', out.height= '500px', echo=FALSE, eval=TRUE}
x <- seq(-6, 10, by=.01)
lx <- length(x)
sig <- c(.3,.5,1,2,3)
ls <- length(sig)
...
:::
::::
# Temps continu, espace d'états infini indénombrable {background-image="https://images.squarespace-cdn.com/content/v1/57d1f16d197aea6d03f08252/1473461284521-JRD0QNGA8YP01V2C3IAQ/Otter_schwimmend_Tusche.gif?format=2500w" background-size="cover" background-opacity="0.4"}
...
- attribué à R.Brown en 1827; approches mathématiques : L.Bachelier en 1901 et N.Wiener en 1923
...
- l'existence de $B$ garantie comme limite $B_t = tN_0+ \sum_{k=1}^{+\infty} \frac{\sqrt 2}{2\pi k}\left[N_k (\cos(2\pi k t)-1) + N'_k\sin(2\pi k t)\right]$ où $(N_k)_{k\in\mathbb N}$ et $(N_k)_{k\in\mathbb N}$ sont deux familles indépendantes de variables IID $\mathcal N(0,1)$
